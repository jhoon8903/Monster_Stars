...
namespace Script.RewardScript
{
    public class CommonRewardManager : MonoBehaviour
    {
        ...
        private List<CommonData> powerUps = null;
        ...
        private IEnumerator CommonChance(int greenChance, int blueChance, int purpleChance, string forcedColor)
        {
            if (gameManager.wave == 11)
            {
                ...
                blueChance = 55;
                purpleChance = 15;
                powerUps = CommonPowerList(greenChance, blueChance, purpleChance, "blue");
                CommonDisplay(powerUps);
                yield return null;
            }
            else
            {
                powerUps = CommonPowerList(greenChance, blueChance, purpleChance, forcedColor);
                CommonDisplay(powerUps);
                yield return null;
            }
        }
       ...
       private bool IsValidOption(CommonData powerUp, ICollection<int> selectedCodes)
            {
                ...
                switch (powerUp.Type)
                {
                    ...
                    case CommonData.Types.Exp:
                        if (_expPercentage > 30) return false; // Make sure the EXP increment does not exceed 30%
                        break;
                    case CommonData.Types.AddRow:
                        if (gameManager.wave != 11 || gameManager.wave != 21 && _addRowCount >= 2) return false; // Show row extra reward only after boss stage, up to 2 times
                        break;
                    case CommonData.Types.Slow:
                        if (_slowCount <= 3) return false; // Displays the enemy movement speed reduction effect up to 3 times
                        break;
                    case CommonData.Types.NextStage:
                        if (_nextStageMembersSelectCount <= 3) return false; // Only use up to 3 next stage character upgrades
                        break;
                    case CommonData.Types.StepDirection:
                        if (_diagonalMovement) return false; // If diagonal movement is possible, don't show this option
                        break;
                    case CommonData.Types.Match5Upgrade:
                        if (_5MatchUpgradeOption) return false; // Don't show this option if 5 matching upgrade option is enabled
                        break;
                    case CommonData.Types.StepLimit:
                        if (_permanentIncreaseMovementCount) return false; // don't show this option if permanent move count increment is enabled
                        break;
                    case CommonData.Types.CastleRecovery:
                        if (_recoveryCastle) return false; // Castle recovery can only be used once
                        break;
                    case CommonData.Types.GroupLevelUp:
                        if (_characterGroupLevelUpIndexes.Contains(powerUp.Property[0])) return false; // Do not display GroupLevelUp options for groups where LevelUpPattern is executed
                        break;
                }
       ...
       private void CommonDisplayText(Button commonButton, TMP_Text powerText, TMP_Text powerCode, Image btnBadge ,CommonData powerUp)
        {
            switch (powerUp.Type)
            {
                case CommonData.Types.Exp: powerText.text = $"Acquire additional Exp {powerUp.Property[0]}% when killing an enemy."; break;
                case CommonData.Types.Slow: powerText.text = $"Enemy movement speed is slowed by {powerUp.Property[0]}%. (up to 45%)"; break;
                case CommonData.Types.GroupDamage: powerText.text = $"Increases the damage of the entire character group by {powerUp.Property[0]}%."; break;
                case CommonData.Types.GroupAtkSpeed: powerText.text = $"Increases the attack speed of the entire character group by {powerUp.Property[0]}%."; break;
                case CommonData.Types.Step: powerText.text = $"Move count increases by {powerUp.Property[0]} steps."; break;
                case CommonData.Types.StepLimit: powerText.text = $"{powerUp.Property[0]} steps are added to the movement count for each wave."; break;
                case CommonData.Types.StepDirection: powerText.text = $"{powerUp.Property[0]} steps are added to the movement count for each wave."; break;
                case CommonData.Types.RandomLevelUp: powerText.text = $"Level up {powerUp.Property[0]} characters on the puzzle by 1 level."; break;
                case CommonData.Types.GroupLevelUp: var groupLevelUpName = characterManager.UnitGroupsCheck(powerUp.Property[0]); powerText.text = $"Level 0 characters in the {groupLevelUpName} group on the puzzle will level up by 1"; break;
                case CommonData.Types.LevelUpPattern: var groupName = characterManager.UnitGroupsCheck(powerUp.Property[0]); powerText.text = $"Level 0 characters in Group_{groupName} appear as level 1."; break;
                case CommonData.Types.CastleRecovery: powerText.text = $"If you take no damage from the wave, the castle's health is restored by {powerUp.Property[0]}."; break;
                case CommonData.Types.CastleMaxHp: powerText.text = $"Increases the max HP and HP of the castle by {powerUp.Property[0]}."; break;
                case CommonData.Types.Match5Upgrade: powerText.text = "When 5 matches are made, the level of the middle character increases by 1 additional level."; break;
                case CommonData.Types.NextStage: powerText.text = $"You can take {powerUp.Property[0]} additional characters when initializing characters."; break;
                case CommonData.Types.Gold: powerText.text = $"At 5 matches, additional gold is obtained as much as {powerUp.Property[0]}."; break;
                case CommonData.Types.AddRow: powerText.text = $"A horizontal {powerUp.Property[0]} line is added."; break;
        ...
        {
            var findCharacterManger = FindObjectOfType<CharacterManager>();
            switch (selectedReward.Type)
            {
                case CommonData.Types.AddRow: gridManager.AddRow(); _addRowCount += 1; break;                                   // Row 추가 강화 효과
                case CommonData.Types.GroupDamage: findCharacterManger.IncreaseGroupDamage(selectedReward.Property[0]); break;     // 전체 데미지 증가 효과
                case CommonData.Types.GroupAtkSpeed: findCharacterManger.IncreaseGroupAtkRate(selectedReward.Property[0]); break;  // 전체 공격 속도 증가 효과
                case CommonData.Types.Step: countManager.IncreaseRewardMoveCount(selectedReward.Property[0]); break;            // 카운트 증가
                case CommonData.Types.StepLimit: countManager.PermanentIncreaseMoveCount(selectedReward.Property[0]); _permanentIncreaseMovementCount = true; break; // 영구적 카운트 증가
                case CommonData.Types.StepDirection: swipeManager.EnableDiagonalMovement(); _diagonalMovement = true; break;    // 대각선 이동
                case CommonData.Types.RandomLevelUp: findCharacterManger.RandomCharacterLevelUp(selectedReward.Property[0]); break;// 랜덤 케릭터 레벨업
                case CommonData.Types.GroupLevelUp: findCharacterManger.CharacterGroupLevelUp(selectedReward.Property[0]);
                    Debug.Log($"ProcessCommonReward: {selectedReward.Property[0]}");break;  // 케릭터 그룹 레벨업
                
                case CommonData.Types.LevelUpPattern: findCharacterManger.PermanentIncreaseCharacter(selectedReward.Property[0]); _characterGroupLevelUpIndexes.Add(selectedReward.Property[0]); break; // 기본 2레벨 케릭터 생성
                case CommonData.Types.Exp: expManager.IncreaseExpBuff(selectedReward.Property[0]); _expPercentage += 5; break;  // 경험치 5% 증가
                case CommonData.Types.CastleRecovery: gameManager.RecoveryCastle = true; _recoveryCastle = true; break;         // 성 체력 회복
                case CommonData.Types.Match5Upgrade: matchManager.match5Upgrade = true; _5MatchUpgradeOption = true; break;     // 5매치 패턴 업그레이드
                case CommonData.Types.Slow: enemyManager.DecreaseMoveSpeed(selectedReward.Property[0]); _slowCount += 1; break; // 적 이동속도 감소 
                case CommonData.Types.NextStage: spawnManager.nextCharacterUpgrade(selectedReward.Property[0]); _nextStageMembersSelectCount += 1; break; // 보드 초기화 시 케릭터 상속되는 케릭터 Count 증가
                case CommonData.Types.Gold: Debug.LogWarning($"Unhandled reward type: {selectedReward.Type}"); break;
            ...
            selectedReward._chosenProperty = null;
